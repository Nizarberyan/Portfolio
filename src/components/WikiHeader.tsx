  ...
  return (
    <div className="vector-header-container">
      <header className="vector-header mw-header">
        {/* If Mobile Search is Active: Full Width Search Header */}
        {isMobileSearchOpen ? (
          <div className="flex items-center w-full gap-2 sm:hidden py-1">
            <div className="flex-1 flex items-center bg-[#eaecf0] dark:bg-[#202122] border border-[#a2a9b1] dark:border-[#54595d] rounded-xs px-2.5 h-[36px]">
              ...
              <input
                ...
                className="w-full bg-transparent border-none outline-none text-sm text-[#202122] dark:text-[#eaecf0]"
              />
              {searchQuery && (
                <button
                  type="button"
                  onClick={() => onSearchChange('')}
                  className="text-xs text-gray-400 p-1 cursor-pointer"
                >
                  ✕
                </button>
              )}
            ...
            <button
              ...
              className="px-2 py-1 text-xs font-semibold text-[#3366cc] dark:text-[#6ea8fe] cursor-pointer"
            >
              Cancel
            </button>
          </div>
          ...
          <>
            ...
            <div className="flex items-center gap-2 sm:gap-3 shrink-0">
              <button
                ...
                className="p-1.5 text-[#54595d] dark:text-[#a2a9b1] hover:bg-[#eaecf0] dark:hover:bg-[#27292d] rounded cursor-pointer"
            ...
            {/* Right Side: Mobile Search Trigger + Appearance + Actions */}
            <div className="flex items-center gap-1 shrink-0">
              {/* Mobile Search Icon Button */}
              <button
                onClick={() => setIsMobileSearchOpen(true)}
                className="sm:hidden p-1.5 text-[#54595d] dark:text-[#a2a9b1] hover:bg-[#eaecf0] dark:hover:bg-[#27292d] rounded cursor-pointer"
                aria-label="Search"
              >
      ...
      {/* Mobile Search Results Overlay (When user is typing in mobile search) */}
      {isMobileSearchOpen && searchQuery && (
        <div className="fixed inset-x-0 top-[52px] bottom-0 bg-black/40 z-40 sm:hidden">
          <div className="bg-white dark:bg-[#202122] border-b border-[#a2a9b1] dark:border-[#54595d] max-h-[70vh] overflow-y-auto divide-y divide-gray-100 dark:divide-gray-800 p-2 text-xs">
            <div className="p-2 bg-[#f8f9fa] dark:bg-[#27292d] text-[#54595d] font-semibold text-[11px]">
              Matching Sections & Skills
            </div>
            {suggestions.map((item, idx) => (
              <button
                key={idx}
                onClick={() => handleSelectSuggestion(item.sectionId, item.label)}
                className="w-full text-left p-3 hover:bg-[#eaecf0] dark:hover:bg-[#27292d] flex items-center justify-between text-[#202122] dark:text-[#eaecf0] cursor-pointer"
              >
                <span className="font-medium text-sm">{item.label}</span>
                <span className="text-[10px] px-1.5 py-0.5 rounded bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-300 font-mono">
                  {item.type}
